...
public class OrderServiceQueryTests
{
    ...
    [Fact]
    public async Task GetOrders_Page1_IncludesMostRecentlyCreatedOrder()
    {
        using var db = TestSetup.CreateContext();
        var service = TestSetup.CreateOrderService(db);
        var customer = TestSetup.AddCustomer(db);
        ...
        for (var i = 0; i < 25; i++)
        {
            var order = new Order { CustomerId = customer.Id, Status = OrderStatus.Confirmed, CreatedAt = DateTime.UtcNow.AddMinutes(-i) };
            db.Orders.Add(order);
            if (i == 0) newest = order;
        }
        db.SaveChanges();
